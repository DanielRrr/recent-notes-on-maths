...
Given a functor $\mathcal{X} \to \mathcal{D}$ from the category of atoms $\mathcal{X}$ to a balanced dialogue category.
Then one has:
\begin{theorem}{Soundness}

  Every derivation $\pi$ of a sequent:
  \[
  A_1 \otimes \ldots \otimes A_n \vdash A
  \]
  in ribbon tensorial logic is intepreted as a morphism
  \[
  [\pi] : [A_1] \otimes \ldots \otimes [A_n] \vdash [A]
  \]
  in the balanced dialogue category $\mathcal{D}$. Besides, the interpretation $[\pi]$ provides the invariant of
  a derivation tree.
\end{theorem}

The interpretation $[\pi]$ of a derivation tree $\pi$ is defined by structural induction. In particular, the interpretation of the exchange rule
  \begin{prooftree}
    \AxiomC{$A_1, \ldots, A_n \vdash B$}
    \AxiomC{$g \in {\bf G}_n$}
    \BinaryInfC{$A_{g \triangleleft 1}, \ldots, A_{g \triangleleft n} \vdash B$}
...
is interpreted the following way:
  \begin{prooftree}
    \AxiomC{$[A_1] \otimes \ldots \otimes [A_n] \xrightarrow{[\pi] } [B]$}
    \AxiomC{$g \in {\bf G}_n$}
    \BinaryInfC{$A_{g \triangleleft 1} \otimes \ldots \otimes A_{g \triangleleft n} \xrightarrow{\theta} [A_1] \otimes \ldots \otimes [A_n] \xrightarrow{[\pi] } [B]$}
  \end{prooftree}
where $\theta$ is a morphism associated to $g \in {\bf G}_n$ acting on $[A_1] \otimes \ldots \otimes [A_n]$.

\section{Proof-as-Tangle theorem}

There is another way to formulate the soundness theorem. One can state that the free balanced dialogue category ${\bf dialogue}(\mathcal{X})$
generated by $\mathcal{X}$ coincides with a category of tensorial formulas and derivation trees modulo the equational theory of ribbon tensorial logic.
Then one can deduce the functor
\[
[.] : {\bf dialogue}(\mathcal{X}) \to {\bf ribbon}(\mathcal{X} \sqcup {\bf 1}) 
\]
that transports every tensorial proof net $\pi$ into a topological tangle $[\pi]$.
...
\begin{proof}
  Consider two cut-free derivation trees $\pi_1 : A \vdash B$ and $\pi_2 : A \vdash B$ in tensorial ribbon logic inducing the same tangle
  $[\pi_1] = [\pi_2]$ modulo topological deformation in the free ribbon category ${\bf ribbon}(\mathcal{X} \sqcup {\bf 1})$.
  We show that $\pi_1 \leftrightsquigarrow \pi_2$ and conclude. We proceed by induction on the number of strands in the tangle. By the focusing theorem,
  we know that the proofs $\pi_1$ and $\pi_2$ are equal modulo the equational theory to the following derivations:
  \begin{prooftree}
  \AxiomC{$\pi'_1$}
  \noLine
  \UnaryInfC{$\:\:$}
  ...
  \noLine
  \UnaryInfC{$\:\:$}
  \UnaryInfC{$A_1, \ldots, A_n \vdash B$}
  \end{prooftree}
  \begin{prooftree}
  \AxiomC{$\pi'_2$}
  \noLine
  ...
  \UnaryInfC{$A_1, \ldots, A_n \vdash B$}
  \end{prooftree}
where each $A_i$ is either a negation or an atom, followed by the same sequence of left introduction of tensor or unit. Suppose that $B = \bot$, then $\bot$
was either introduced by:
\begin{itemize}
  \item the left introduction of a left negation $X \multimap \bot$,
  \item the left introduction of a right negation $\bot \multimapinv X$.
\end{itemize}
By symmetry, we may suppose that the last rule introduces a left negation $X \multimap \bot$. In this case, $\pi'_1$ is equal to
  \begin{prooftree}
  \AxiomC{$\pi''_1$}
  \noLine
  \UnaryInfC{$\:\:$}
  ...
  \noLine
  \UnaryInfC{$\:\:$}
  \UnaryInfC{$X_1, \ldots, X_{n-1} \vdash X$}
  \RightLabel{$\multimap_L$}
  \UnaryInfC{$X_1, \ldots, X_{n-1}, X \multimap \bot \vdash \bot$}
  \RightLabel{$[g]$}
  \UnaryInfC{$A_1, \ldots, A_n \vdash \bot$}
  \end{prooftree}
In turn, $\pi'_2$ is equal to the following proof:
\begin{prooftree}
  \AxiomC{$\pi''_2$}
  \noLine
  \UnaryInfC{$\:\:$}
  ...
  \noLine
  \UnaryInfC{$\:\:$}
  \UnaryInfC{$Y_1, \ldots, Y_{n-1} \vdash X$}
  \RightLabel{$\multimap_L$}
  \UnaryInfC{$Y_1, \ldots, Y_{n-1}, X \multimap \bot \vdash \bot$}
  \RightLabel{$[h]$}
  \UnaryInfC{$A_1, \ldots, A_n \vdash \bot$}
\end{prooftree}
So we conclude that 
  ...
  \UnaryInfC{$X_1, \ldots, X_{n-1}, X \multimap \bot \vdash \bot$}
  \end{prooftree}
  induced the same topological tangle as the derivation tree below:
\begin{prooftree}
  \AxiomC{$\pi''_2$}
  \noLine
  \UnaryInfC{$\:\:$}
  ...
  \UnaryInfC{$X_1, \ldots, X_{n-1}, X \multimap \bot \vdash \bot$}
\end{prooftree}  
All the formulas $A_i$ are either negated or atoms, and thus $g^{-1} \circ h \in {\bf G}_n$ is of the form $f \otimes 1$.
From this follows that the proof
\begin{prooftree}
  \AxiomC{$\pi''_1$}
  \noLine
  \UnaryInfC{$\:\:$}
...
ha\begin{prooftree}
  ...
  \UnaryInfC{$X_1, \ldots, X_{n-1} \vdash X$}
  \RightLabel{$[f]$}
  \UnaryInfC{$Y_1, \ldots, Y_{n-1} \vdash Y$}
\end{prooftree}
They are therefore equal by the induction hypothesis.
$\pi'_1$ is thus equal to the proof
\begin{prooftree}
  \AxiomC{$\pi''_2$}
  \noLine
  \UnaryInfC{$\:\:$}
  ...
  \RightLabel{$\multimap_L$}
  \UnaryInfC{$Y_1, \ldots, Y_{n-1}, X \multimap \bot \vdash \bot$}
  \RightLabel{$[f \otimes 1]$}
  \UnaryInfC{$X_1, \ldots, X_{n-1}, X \multimap \bot \vdash \bot$}
  \RightLabel{$[g]$}
  \UnaryInfC{$A_1, \ldots, A_n \vdash \bot$}
\end{prooftree}
that can be rewritten into the proof:
\begin{prooftree}
  \AxiomC{$\pi''_2$}
  \noLine
  \UnaryInfC{$\:\:$}
  ...
  \UnaryInfC{$A_1, \ldots, A_n \vdash \bot$}
\end{prooftree}
and therefore $\pi_1$ and $\pi_2$ are equal.
\end{proof}
...
\]
produced by the derivation trees $\pi_1$ and $\pi_2$ in the tensor formula $B = B_1 \otimes B_2$,
and all the hypotheses are either negated or atomic. One may suppose that the last rule of $\pi_1$ introduces a tensor on the right.
$\pi_1$ factors as follows:
\begin{prooftree}
  \AxiomC{$\pi_{11}$}
  \noLine
  \UnaryInfC{$\:\:$}
  ...
  \noLine
  \UnaryInfC{$\:\:$}
  \UnaryInfC{$A_1, \ldots, A_k \vdash B_1$}
  \AxiomC{$\pi_{12}$}
  \noLine
  \UnaryInfC{$\:\:$}
  \noLine
  ...
  \noLine
  \UnaryInfC{$\:\:$}
  \UnaryInfC{$A_{k+1}, \ldots, A_n \vdash B_2$}
  \RightLabel{$\otimes_R$}
  \BinaryInfC{$A_1, \ldots, A_k, A_{k+1}, \ldots, A_n \vdash B_1 \otimes B_2$}
\end{prooftree}
The fact that tangles $[\pi_1]$ and $[\pi_2]$ are equal modulo deformations implies $\pi_2$ splits similarly into $\pi_{21}$ and $\pi_{22}$.
Besides, $[\pi_{11}]$ and $[\pi_{21}]$ are equal modulo transformations as well as $[\pi_{12}]$ and $[\pi_{22}]$. Thus, $\pi_{11}$ and $\pi_{21}$
are equal modulo commuting conversions as well as $\pi_{12}$ and $\pi_{22}$. Therefore, $\pi_1$ and $\pi_2$ are equal modulo commuting conversions.

The proof-as-tangle theorem provides a coherence theorem for balanced monoidal categories, such as the category ${\bf Mod}(H)$ of finite 
and infinite dimensional $H$-modules associated to a ribbon Hopf algebra $H$. For example, if one needs to establish that the following diagram commutes in every balanced dialogue category
\[
\begin{tikzcd}
  \bot \multimapinv (\bot \multimapinv A) \ar[rrrr, "\bot \multimapinv \operatorname{turn}_A"] &&&& \bot \multimapinv (A \multimap \bot) \\
  (\bot \multimapinv A) \multimap \bot \ar[u, "\operatorname{turn}_{\bot \multimapinv A}"] && A \ar[rr, "\eta_A"'] \ar[ll, "\eta'_A"] && \bot \multimapinv (A \multimap \bot) \ar[u, "\theta_{\bot} \multimapinv (A \multimap \bot)"']
\end{tikzcd}
\]
where $\eta_A$ and $\eta'_A$ are units of the corresponding double negation monads and $\operatorname{turn}$ is the canonical isomorphism between two negations.
Commutativity of the above diagram, in turn, means the following derivation trees $\pi_1$ and $\pi_2$ are equal modulo commuting conversions:
\begin{prooftree}
  \AxiomC{$A \vdash A$}
  \RightLabel{$\multimap_L$}
  \UnaryInfC{$A, A \multimap \bot \vdash \bot$}
  \RightLabel{[\bf torsion]}
  \UnaryInfC{$A, A \multimap \bot \vdash \bot$}
  \RightLabel{$\multimapinv_R$}
  \UnaryInfC{$A \vdash \bot \multimapinv (A \multimap \bot)$}
\end{prooftree}
\begin{prooftree}
  \AxiomC{$A \vdash A$}
  \RightLabel{$\multimapinv_L$}
  \UnaryInfC{$\bot \multimapinv A, A \vdash \bot$}
  \RightLabel{$\multimap_R$}
  \UnaryInfC{$A \vdash (\bot \multimapinv A) \multimap \bot$}
  \AxiomC{$A \vdash A$}
  \RightLabel{$\multimap_L$}
  \UnaryInfC{$A, A \multimap \bot \vdash \bot$}
  \RightLabel{$[\sigma \circ \theta_1]$}
  \UnaryInfC{$A \multimap \bot, A \vdash \bot$}
  \RightLabel{$\multimapinv_R$}
  \UnaryInfC{$A \multimap \bot \vdash \bot \multimapinv A$}
  \RightLabel{$\multimap_L$}
  \UnaryInfC{$A \multimap \bot, (\bot \multimapinv A) \multimap \bot \vdash \bot$}
  \RightLabel{$[\sigma \circ \theta_1]$}
  \UnaryInfC{$(\bot \multimapinv A) \multimap \bot, A \multimap \bot \vdash \bot$}
  \RightLabel{$\multimapinv_R$}
  \UnaryInfC{$(\bot \multimapinv A) \multimap \bot \vdash \bot \multimapinv (A \multimap \bot)$}
  \RightLabel{\bf cut}
  \BinaryInfC{$A \vdash \bot \multimapinv (A \multimap \bot)$}
\end{prooftree}
where ${\bf torsion} = \theta\langle 2 \rangle$ of ${\bf G}_2$ twists the two hypotheses of the proof, or twists the conclusion $\bot$
with an angle $2\pi$. One can construct the ribbon tangles $[\pi_1]$ and $[\pi_2]$ associated to $\pi_1$ and $\pi_2$ by structural induction,
so a proof $\pi$ of a sequent $A_1, \ldots, A_n \vdash B$ as a form $[\pi]$ on the object $[A_1] \otimes \ldots \otimes [A_n] \otimes [B]^*$
in the category ${\bf ribbon}(\mathcal{X} \sqcup {\bf 1})$. A \emph{form} on an object $A$ in a ribbon category $\mathcal{C}$
is a morphism from $A$ to the tensorial unit $I$. If we use the notation $\hbar = \bot$ for the tensorial pole object of
${\bf ribbon}(\mathcal{X} \sqcup {\bf 1})$ and $h = \bot^*$ for its dual, so we can describe $[\pi_1]$ and $[\pi_2]$
as a sequence of local transformations performed on norms. So $[\pi_1]$ looks like this
\begin{prooftree}
  \AxiomC{$A, A^*$}
  \RightLabel{$\multimap_L$}
  \UnaryInfC{$A, A^* \otimes \hbar, h$}
  \RightLabel{[\bf torsion]}
  \UnaryInfC{$A, A^* \otimes \hbar, h$}
  \RightLabel{$\multimapinv_R$}
  \UnaryInfC{$A, A^* \otimes \hbar \otimes h$}
\end{prooftree}
with the following topological tangle:
  \begin{center}
  \includegraphics[scale=0.3]{tangle-1.png}
  \end{center}
Whereas $[\pi_2]$ looks like this
  \begin{center}
  \includegraphics[scale=0.35]{tangle-2.png}
  \end{center}
