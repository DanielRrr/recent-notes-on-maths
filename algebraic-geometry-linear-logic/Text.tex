...
One can describe the construction of the above adjunction explicitly in many cases, for example when $R = k$ is an algebraic closed field, see \cite{murfet2015sweedler}.
...
The category ${\bf Points}(X)$ comes with a functor $\pi_X : {\bf Points}(X) \to {\bf Ring}$. $\pi_X$ is called the functor of points of $X$ and 
...
We construct the category ${\bf PshMod}$ of presheaves of modules and forward morphisms as follows. A \emph{presheaf of modules} $(X,M)$
...
A proof of the statement based on a 2-categorical construction of the $\mathcal{O}_Y$-module $f_!(M)$. Note that
...
\begin{proof}
One uses the fact that the functor $\pi : {\bf Mod} \to {\bf Ring}$ is a bifibration, in particular,
